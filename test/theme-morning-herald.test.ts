...
import { morningHeraldTheme } from "../src/themes/morning-herald";
...
// morning-herald doesn't render footer settings (see night-wire), but every
...
describe("morningHeraldTheme", () => {
	test("has the expected id/name", () => {
		expect(morningHeraldTheme.id).toBe("morning-herald");
		expect(morningHeraldTheme.name).toBe("Morning Herald");
	});

	test("includes the masthead wordmark", () => {
		const html = morningHeraldTheme.render({
			stories: [story()],
			isCatchup: false,
			dateLabel,
			footer: footer(),
		});
		expect(html).toContain("The HN Daily");
	});
	...
	test("renders every story uniformly — no distinct lead-story treatment", () => {
		...
		expect(titleFontSizes).toEqual(["17", "17", "17"]);
	});
	...
	test("shows the hostname of the story's linked URL", () => {
		...
		expect(html).toContain("nytimes.com &mdash; 100 points");
	});
