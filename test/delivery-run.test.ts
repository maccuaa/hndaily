...
function mailerConfig(): MailerConfig {
  return { host: "smtp.example.com", port: 465, username: "u", password: "p", from: "hndaily@snowcastle.ca" };
}

function story(overrides: Partial<Story> = {}): Story {
  return { hnId: 1, title: "A story", url: "https://example.com", points: 100, numComments: 10, createdAt: 1_700_000_000, ...overrides };
}
...
describe("runDeliveryRun", () => {
  test("on success: records a success run, records sent stories, and pings the heartbeat", async () => {
    ...
    await runDeliveryRun(
      { db, config: config(), mailerConfig: mailerConfig(), heartbeatUrl: "https://hc-ping.com/abc" },
      {
