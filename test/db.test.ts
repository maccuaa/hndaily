...
describe("db", () => {
  ...
  test("getLastSuccessfulRunTime ignores failed runs and picks the latest success", () => {
    const db = openDb(":memory:");
    recordDeliveryRun(db, { startedAt: 100, status: "success", isCatchup: false, storiesSentCount: 1 });
    recordDeliveryRun(db, { startedAt: 300, status: "failure", isCatchup: false, storiesSentCount: 0 });
    recordDeliveryRun(db, { startedAt: 200, status: "success", isCatchup: false, storiesSentCount: 1 });
  ...
  test("recordDeliveryRun returns incrementing row ids", () => {
    const db = openDb(":memory:");
    const id1 = recordDeliveryRun(db, { startedAt: 100, status: "success", isCatchup: false, storiesSentCount: 0 });
    const id2 = recordDeliveryRun(db, { startedAt: 200, status: "success", isCatchup: false, storiesSentCount: 0 });
    expect(id2).toBeGreaterThan(id1);
  });

  test("recordSentStories makes ids show up in getAllSentStoryIds", () => {
    const db = openDb(":memory:");
    const runId = recordDeliveryRun(db, { startedAt: 100, status: "success", isCatchup: false, storiesSentCount: 2 });
  ...
  test("the same hn_id can be recorded again under a different delivery run (composite key)", () => {
    ...
    const db = openDb(":memory:");
    const run1 = recordDeliveryRun(db, { startedAt: 100, status: "success", isCatchup: false, storiesSentCount: 1 });
    const run2 = recordDeliveryRun(db, { startedAt: 200, status: "success", isCatchup: false, storiesSentCount: 1 });
  ...
  test("recordSentStories stores a null url for self-posts", () => {
    const db = openDb(":memory:");
    const runId = recordDeliveryRun(db, { startedAt: 100, status: "success", isCatchup: false, storiesSentCount: 1 });
